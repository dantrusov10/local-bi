...
const COLORS = ['#55c6ff', '#4a7cff', '#44e1cf', '#7a8dff', '#3ed598', '#b184ff', '#ffb85c']

function maxVal(data, secondary = []) {
  return Math.max(...data.map((d) => Number(d.value || 0)), ...secondary.map((d) => Number(d.value || 0)), 1)
}
    ...
    <div className="panel glass">
            ...
            <div key={d.label + i} className="bar-col">
              <div className="bar-stack">
                ...
                {mode === 'stacked' && safeSecondary[i] && (
                  <div className="bar overlay" style={{ height: Math.max(8, (Number(safeSecondary[i].value || 0) / max) * 120) + 'px', background: color(i + 3) }} />
                )}
              </div>
              <div className="bar-label" title={d.label}>{d.label}</div>
              <div className="bar-value">{d.value}{safeSecondary[i]?.value ? ` / ${safeSecondary[i].value}` : ''}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {(mode === 'line' || mode === 'area' || mode === 'combo') && !!safe.length && (
        <div className="svg-chart-wrap">
          <svg viewBox="0 0 640 280" className="svg-chart">
            <defs>
            ...
            {[0,1,2,3].map((r) => <line key={r} x1="40" x2="620" y1={40 + r*55} y2={40 + r*55} className="svg-grid-line" />)}
            {mode === 'combo' && safe.map((d, i) => {
              const x = 50 + i * (560 / Math.max(safe.length - 1, 1))
              const h = Math.max(8, (Number(d.value || 0) / max) * 120)
              return <rect key={'bar'+i} x={x - 12} y={220 - h} width="24" height={h} rx="8" fill="rgba(74,124,255,0.28)" />
            })}
            {mode === 'area' && <path d={buildAreaPath(safe, max)} fill="url(#areaFill)" stroke="none" />}
            <path d={buildLinePath(safe, max)} fill="none" stroke="#55c6ff" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
            {!!safeSecondary.length && <path d={buildLinePath(safeSecondary, max)} fill="none" stroke="#44e1cf" strokeWidth="3" strokeDasharray="8 6" strokeLinecap="round" />}
            {safe.map((d, i) => {
              const x = 50 + i * (560 / Math.max(safe.length - 1, 1))
      ...
      {(mode === 'pie' || mode === 'donut') && !!safe.length && (
        <div className="pie-wrap">
          <svg viewBox="0 0 260 260" className="pie-svg">
            {buildPieSlices(safe, mode === 'donut').map((slice, i) => <path key={i} d={slice.d} fill={color(i)} />)}
...
function buildAreaPath(data, max) {
  ...
  return `${line} L ${endX} 220 L 50 220 Z`
}
