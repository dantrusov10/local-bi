...
function similarityByName(a, b) {
  ...
  if (aliases.some(([m, n]) => (x === m && y === n) or (x === n && y === m))) return 0.8
  return 0
}

function overlapScore(leftRows, rightRows, leftColumn, rightColumn) {
  const left = new Set(leftRows.map((r) => normalizeKey(r[leftColumn])).filter(Boolean))
  const right = new Set(rightRows.map((r) => normalizeKey(r[rightColumn])).filter(Boolean))
...
function relationType(leftRows, rightRows, leftColumn, rightColumn) {
  const leftValues = leftRows.map((r) => normalizeKey(r[leftColumn])).filter(Boolean)
  const rightValues = rightRows.map((r) => normalizeKey(r[rightColumn])).filter(Boolean)
...
export function validateRelation(leftTable, rightTable, relation) {
  const leftKeys = leftTable.rows.map((r) => normalizeKey(r[relation.leftColumn])).filter(Boolean)
  const rightKeys = rightTable.rows.map((r) => normalizeKey(r[relation.rightColumn])).filter(Boolean)
  const rightSet = new Set(rightKeys)

  let matched = 0
  for (const key of leftKeys) {
    if (rightSet.has(key)) matched += 1
  }
